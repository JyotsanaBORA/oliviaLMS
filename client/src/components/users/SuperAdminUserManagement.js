      ...
      <div className="p-6">
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={() => setRoleFilter('sub_agent')}
              className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${
                roleFilter === 'sub_agent'
                  ? 'bg-amber-100 text-amber-700 border border-amber-300'
                  : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}
            >
              Sub-Agents ({users.filter(u => u.role === 'sub_agent' || u.role === 'vendor_agent').length})
            </button>
            <button
                      ...
                      <td className="py-4 px-4">
                        <div>
                          <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getRoleColor(targetUser.role)}`}>
                            {targetUser.role === 'sub_agent' ? 'SUB-AGENT' : targetUser.role.toUpperCase()}
                          </span>
                          <div className="text-xs text-gray-600 mt-1">
                            {getRoleDescription(targetUser.role)}
                          </div>
                          {Array.isArray(targetUser.assignedDids) && targetUser.assignedDids.length > 0 && (
                            <div className="text-[11px] font-mono text-indigo-700 bg-indigo-50 px-1.5 py-0.5 rounded mt-1 inline-block border border-indigo-200">
                              DIDs: {targetUser.assignedDids.join(', ')}
                            </div>
                          )}
                        </div>
                      </td>
