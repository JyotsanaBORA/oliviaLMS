...
router.post('/:id/load', protect, authorize('dom_admin', 'dom_superadmin'), async (req, res) => {
  try {
    ...
    const lead = await DomWebsiteLead.findOneAndUpdate(
      ...
      { $set: { status: 'loaded', loadedBy: req.user._id, loadedAt: assignedAt }, $push: { assignmentHistory: { agent: req.user._id, assignedAt } } },
      { new: true }
    ).lean();
...
router.post('/:id/assign', protect, authorize('dom_admin', 'dom_superadmin'), async (req, res) => {
  try {
    ...
    const lead = await DomWebsiteLead.findOneAndUpdate(
      { _id: req.params.id, status: 'new' },
      { $set: { status: 'loaded', loadedBy: agentId, loadedAt: assignedAt }, $push: { assignmentHistory: { agent: agentId, assignedAt } } },
      { new: true }
    ).lean();
