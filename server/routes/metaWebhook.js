/**
 * Meta Lead Ads Webhook — thin router.
 *
 * All logic lives in:
 *   controller  → server/controllers/metaWebhookController.js
 *   graph API   → server/services/metaLeadService.js
 *   field parser → server/utils/metaFieldParser.js
 *   signature   → server/middleware/metaSignature.js
 *
 * Routes:
 *   GET  /api/webhook/meta  — Meta one-time verification challenge
 *   POST /api/webhook/meta  — Meta lead notification events
 */

const express                        = require('express');
const { verify, receive }            = require('../controllers/metaWebhookController');
const { validateMetaSignature }      = require('../middleware/metaSignature');

const router = express.Router();

// Meta verification handshake (no signature needed — it's a GET)
router.get('/', verify);

// Lead notification — validate signature, then hand off to controller
