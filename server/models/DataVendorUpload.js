...
const dataVendorUploadSchema = new mongoose.Schema({
  // ── ViciDial / CSV Fields (identical to AdminUpload) ───────────
  ...
  // ── Run / List tracking ────────────────────────────────────────
  listName:   { type: String, required: true, trim: true, index: true },
  runBatchId: { type: String, required: true, index: true },
  ...
  sharedWith: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    required: [true, 'Shared-with vendor user is required'],
    index: true
  },
  uploadedBy: {
    type: mongoose.Schema.Types.ObjectId,
  ...
  timestamps: true,
  collection: 'datavendoruploads'
});

// ── Compound indexes for common query patterns ─────────────────────
dataVendorUploadSchema.index({ sharedWith: 1, listName: 1 });
dataVendorUploadSchema.index({ sharedWith: 1, runBatchId: 1 });
dataVendorUploadSchema.index({ runBatchId: 1, status: 1 });
dataVendorUploadSchema.index({ sharedWith: 1, listName: 1, runBatchId: 1 });
dataVendorUploadSchema.index({ sharedWith: 1, listName: 1, runNumber: 1 });

module.exports = mongoose.model('DataVendorUpload', dataVendorUploadSchema);
